...
Provides ``make_mock_request`` and ``create_mock_provider_config`` for
gateway integration tests. Extracted from ``conftest.py`` to allow explicit
imports instead of the ``inject_helpers`` fixture anti-pattern.
"""
...
def make_mock_request(
    ...
    req.body = AsyncMock(return_value=b'{"model": "gpt-4"}')  # boundary: test fixture
...
def create_mock_provider_config(
    *,
    ...
    if default_model is None:
        default_model = {"gpt-4": ModelInfo()}  # boundary: test fixture
