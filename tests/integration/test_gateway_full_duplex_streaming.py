...
async def test_gateway_routes_to_full_stream_handler_when_auto_with_eligible_provider():
    ...
    with (
        ...
        mock_provider.proxy_request.return_value = (mock_upstream_response, successful_result)
        ...
        with (
            ...
            if 'content' in call_args.kwargs:
                ...
                content_arg = call_args.args[6]  # 0: client, 1: token, 2: method, 3: headers, 4: path, 5: query_params, 6: content
            # Assert that content is not bytes (should be an async generator)
            assert not isinstance(content_arg, bytes), "Request body should be streamed, not buffered as bytes"
            # Optional: assert it's an async generator (can't reliably check due to mocking)
            # from typing import AsyncGenerator
            # assert isinstance(content_arg, AsyncGenerator), "Content should be an async generator"


if __name__ == "__main__":
    asyncio.run(test_gateway_routes_to_full_stream_handler_when_auto_with_eligible_provider())
