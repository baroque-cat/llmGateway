...
class TestErrorParsingScenarios:
    ...
    async def test_gateway_vs_worker_behavior(self):
        ...
        result_disabled = await provider_disabled._parse_proxy_error(
            mock_response, error_body
        )
        ...
        result_enabled = await provider_enabled._parse_proxy_error(
            mock_response, error_body
        )
        ...
        result_arrearage = await provider_enabled._parse_proxy_error(
            mock_response, arrearage_body
        )
    ...
    async def test_error_parsing_disabled_fallback(self):
        ...
        for status_code, expected_reason in test_cases:
            ...
            mock_response.aread = AsyncMock(
                return_value=b'{"error": {"message": "test"}}'
            )

            result = await provider._parse_proxy_error(
                mock_response, b'{"error": {"message": "test"}}'
            )
            assert (
