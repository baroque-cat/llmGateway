...
def mock_app_state():
    ...
    state.http_client_factory.get_client_for_provider = AsyncMock(return_value=MagicMock())
...
@pytest.mark.asyncio
@pytest.mark.parametrize("error_reason, status_code, expected_penalty, expected_retry", [
    ...
    (ErrorReason.INVALID_KEY, 401, True, False), # Expected: True (Penalty), False (Retry) -> BUT Current logic might differ
    ...
    (ErrorReason.OVERLOADED, 503, False, True), # Special case: Overloaded might force rotation in code
...
async def test_error_behavior_matrix(
    mock_request, mock_provider, mock_app_state, 
    ...
    mock_response.aread = AsyncMock(return_value=f"Error: {error_reason.value}".encode())
    ...
    # Execute
    await _handle_buffered_retryable_request(mock_request, mock_provider, "test_provider")
    ...
    print(f"  - Retried?          : {'YES' if did_retry else 'NO'} ({retry_count} times)")
