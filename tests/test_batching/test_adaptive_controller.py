...
def test_uc03_start_batch_size_at_max_boundary() -> None:
    """
    Pass start_batch_size=50, max_batch_size=50 (boundary value).
    Verify batch_size == 50 (valid and at max boundary).
    """
    controller = _make_controller(
        start_batch_size=50,
        start_batch_delay_sec=15.0,
...
def test_uc04_start_batch_delay_below_min_rejected() -> None:
    """
    Pass start_batch_delay_sec=1.0, min_batch_delay_sec=3.0 to AdaptiveBatchingConfig.
    Verify Pydantic rejects it — start_batch_delay_sec must be within [min, max].
    Out-of-bounds start values are now validated at config level.
    """
    with pytest.raises(ValidationError):
...
def test_uc04_start_batch_delay_at_min_boundary() -> None:
    """
    Pass start_batch_delay_sec=3.0, min_batch_delay_sec=3.0 (boundary value).
    Verify batch_delay == 3.0 (valid and at min boundary).
    """
    controller = _make_controller(
        start_batch_size=30,
        start_batch_delay_sec=3.0,
        min_batch_delay_sec=3.0,
    )
...
def test_uc28_priority_order_rate_limited_then_transient_then_success() -> None:
    ...
    ctrl_rl = _make_controller(start_batch_size=40, start_batch_delay_sec=10.0)
    ...
    ctrl_tr = _make_controller(start_batch_size=40, start_batch_delay_sec=10.0)
    ...
    ctrl_su = _make_controller(start_batch_size=30, start_batch_delay_sec=15.0)
...
# ===========================================================================
# UT-D01..UT-D06: New API contract tests
# ===========================================================================


def test_ut_d01_controller_initializes_from_config_start_batch_size() -> None:
    """
    UT-D01: AdaptiveBatchController(config) initializes batch_size from
...
def test_ut_d02_start_batch_size_capped_at_max_boundary() -> None:
    """
    UT-D02: start_batch_size at max_batch_size boundary results in
...
def test_ut_d03_start_batch_delay_capped_at_min_boundary() -> None:
    """
    UT-D03: start_batch_delay_sec at min_batch_delay_sec boundary results
...
def test_ut_d04_constructor_rejects_initial_batch_size() -> None:
    """
    UT-D04: AdaptiveBatchController(config, initial_batch_size=30) raises TypeError.
    The constructor now only accepts config, not separate initial values.
    """
    config = AdaptiveBatchingConfig()
...
def test_ut_d04_constructor_rejects_initial_batch_delay() -> None:
    """
    UT-D04: AdaptiveBatchController(config, initial_batch_delay=15.0) raises TypeError.
    The constructor now only accepts config, not separate initial values.
    """
    config = AdaptiveBatchingConfig()
    with pytest.raises(TypeError):
...
def test_ut_d06_ramp_up_logic_backward_compatible() -> None:
    """
    UT-D06: Verify ramp-up behavior is unchanged after API refactor.
    Success → batch_size += step, batch_delay -= step.
    """
    controller = _make_controller()
    controller.report_batch_result(_success_results(10))
...
def test_ut_d06_rate_limit_backoff_backward_compatible() -> None:
    """
    UT-D06: Verify rate-limit backoff behavior is unchanged after API refactor.
    Rate-limited → batch_size //= divisor, delay *= multiplier.
    """
    controller = _make_controller(start_batch_size=40, start_batch_delay_sec=10.0)
    results = _rate_limited_results(1) + _success_results(9)
    controller.report_batch_result(results)
    assert controller.batch_size == 20  # 40 // 2
    assert controller.batch_delay == 20.0  # 10 * 2
    assert controller.consecutive_successes == 0
