...
async def test_production_timeout_cascade(
    ...
    .. note::
        ...
        This test takes 120-180 s wall-clock because the server's
        ``response_delay_ms=60000`` and the client's request timeout is
        120 s.  Only 2 batches of 3 (6 requests) can complete within the
        timeout; the remaining 9 time out.

    The server advertises ``max_concurrent_streams=100`` but enforces
    ``internal_concurrency=3`` with a 60 s response delay.  15 concurrent
    requests are sent through a single
    ``CapacityAwareHttp2Transport`` with a 120 s request timeout.

    A background polling task samples ``concurrency_waiters`` every 100 ms
    to capture the peak queue depth.

    Asserts:
        - ``peak_connections == 1`` (exactly one connection opened).
        - At least 6 requests fail with timeout or read error.
        - At most 9 requests succeed.
        - Peak ``concurrency_waiters > 0`` (queued requests observed).
    """
    server: EphemeralHttp2Server = await http2_server_factory(
        max_concurrent_streams=100,
        internal_concurrency=3,
        response_delay_ms=60000,
    )
    collector: MetricsCollector = collector_factory(server)
    collector.start()

    # --- Background polling for concurrency_waiters ---
    peak_waiters = 0
    ...
    async def _poll_waiters() -> None:
        ...
        while not stop_polling.is_set():
            current = server.stats["concurrency_waiters"]
            if current > peak_waiters:
                peak_waiters = current
    ...
    async with httpx.AsyncClient(
        ...
        timeout=httpx.Timeout(120.0, pool=30.0),
    ) as client:
    ...
    print(
        f"\n[test_production_timeout_cascade] "
        f"peak_connections={peak_connections} "
        f"successes={successes} failures={failures} "
        f"peak_waiters={peak_waiters} "
        f"read_timeout={metrics.read_timeout_errors} "
        f"pool_timeout={metrics.pool_timeout_errors} "
        f"other_errors={metrics.other_errors}"
    )

    assert (
        peak_connections == 1
    ), f"Expected peak_connections == 1, got {peak_connections}"
    assert failures >= 6, f"Expected >= 6 failures (timeout cascade), got {failures}"
    assert successes <= 9, (
        f"Expected <= 9 successes (only 2 batches fit in 120 s), " f"got {successes}"
    )
    assert peak_waiters > 0, (
        f"Expected peak_waiters > 0 (queued requests observed), " f"got {peak_waiters}"
    )
