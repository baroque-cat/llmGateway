...
class TestErrorParsingEdgeCases:
    ...
    async def test_concurrent_rule_matching(self):
        ...
        provider = self.create_mock_openai_provider(
            error_config=ErrorParsingConfig(enabled=True, rules=rules)
        )
        ...
        error_body = json.dumps(
            {"error": {"code": "target_pattern", "message": "Target error"}}
        ).encode("utf-8")
        mock_response.aread = AsyncMock(return_value=error_body)
    ...
    async def test_invalid_regex_pattern(self):
        ...
        error_body = json.dumps(
            {"error": {"code": "valid_pattern", "message": "Valid error"}}
        ).encode("utf-8")
        mock_response.aread = AsyncMock(return_value=error_body)
    ...
    async def test_memory_efficiency(self):
        ...
        for i in range(100):
            ...
            error_body = json.dumps(
                {"error": {"code": "test_error", "message": f"Error iteration {i}"}}
            ).encode("utf-8")
            mock_response.aread = AsyncMock(return_value=error_body)
    ...
    async def test_unicode_and_special_characters(self):
        ...
        for message in test_messages:
            error_body = json.dumps(
                {"error": {"message": message, "code": "payment_issue"}}
            ).encode("utf-8")
            mock_response.aread = AsyncMock(return_value=error_body)
