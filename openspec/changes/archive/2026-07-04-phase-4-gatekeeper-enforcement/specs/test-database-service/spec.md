...
The project SHALL provide a `test-database` service in `docker-compose.yml` for local postgres integration tests, using test-safe credentials on a non-conflicting port.
...
- **WHEN** the production `database` service (port 5432) and `test-database` service (port 5433) are both started
