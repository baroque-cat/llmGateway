...
The project SHALL provide a shared `_isolate_metrics_collector` autouse fixture in `tests/unit/metrics/conftest.py` that resets the metrics collector singleton before and after each test using `monkeypatch.delenv()` for pytest-native environment isolation.
...
The project SHALL update `tests/unit/metrics/test_prometheus_backend.py` to rely on the shared `_isolate_metrics_collector` fixture from `tests/unit/metrics/conftest.py` instead of the `_make_unique_name()` counter and `PrometheusMetricsCollector.__new__()` hacks, except where those hacks are needed for tests that intentionally exercise REGISTRY collision scenarios.
