...
`CapacityAwareHttp2Transport(AsyncConnectionPool)` SHALL route requests to connections with available H2 stream capacity, and open new TCP connections when existing connections are full.
...
`CapacityAwareHttp2Transport._assign_requests_to_connections()` SHALL maintain a `connection_request_count` dictionary that counts how many requests are currently assigned to each connection.
...
`CapacityAwareHttp2Transport._max_concurrent_requests(connection)` SHALL return the maximum number of concurrent requests the connection supports.
...
`CapacityAwareHttp2Transport.create_connection()` SHALL pass `on_capacity_update=self._connection_capacity_updated` to all created connections.
...
`CapacityAwareHttp2Transport._connection_capacity_updated()` SHALL re-run `_assign_requests_to_connections()` and close any removed connections.
