...
Six stress test cases that exercise real `httpx.AsyncClient` instances against an ephemeral HTTP/2 server under controlled concurrency, latency, and connection limits. The tests diagnose httpx connection pool behavior — specifically whether it opens new TCP connections when H2 streams on existing connections are exhausted, and what error types are produced under pool saturation, keep-alive churn, and multi-client scenarios.
