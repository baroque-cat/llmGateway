...
Per-provider HTTP/2 stream concurrency cap that prevents the cascading freeze when a provider has a hidden internal concurrency limit lower than the advertised `SETTINGS_MAX_CONCURRENT_STREAMS`.
