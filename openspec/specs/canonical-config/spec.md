...
Provides a single source of truth (`CanonicalConfig` frozen dataclass) for all test configuration values, parsed deterministically from `.env.example` and `config/example_full_config.yaml`. Replaces the implicit `os.environ.setdefault()` fallback pattern and duplicated `_BASE_ENV` dictionaries across config test files.
