# test-infra-foundation

## Purpose

Core pytest configuration and environment variable chain that makes the llmGateway test suite runnable. Covers event loop scope for async fixtures, global per-test timeout, strict marker registration, environment variable template completeness, and conftest fallback defaults.
