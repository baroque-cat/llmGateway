...
Provides a minimal HTTP/2 test server (`EphemeralHttp2Server`) and a multi-source metrics collector (`MetricsCollector`) for stress-testing httpx connection pool behavior under controlled H2 stream limits, response delays, and connection constraints. All components run in-process without external dependencies (no database, no Docker, no API keys).
