# postgres-policy-gatekeeper

## Purpose

Automated policy enforcement for the PostgreSQL integration test suite. Five structural gatekeeper tests (PP1–PP5) validate marker compliance, mock prohibition, script lifecycle integrity, v2 compose syntax, and Makefile delegation. These tests run as part of the `run-postgres-tests.sh` gatekeeper group (collected via `-m "postgres"`) and prevent regression in the postgres test infrastructure.
