# stream-monitor-graceful-shutdown

## Purpose

Guarantees that `StreamMonitor.__anext__()` always calls `_finalize_logging()`
→ `upstream_response.aclose()` on every possible exit path, including those
that bypass `except Exception` blocks (`CancelledError`, `GeneratorExit`).
Prevents permanent HTTP/2 semaphore slot leaks when clients disconnect
mid-stream or tasks are cancelled.
