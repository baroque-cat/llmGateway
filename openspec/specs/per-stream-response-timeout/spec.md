# per-stream-response-timeout

## Purpose

Prevents HTTP/2 stream starvation by enforcing a per-stream deadline on the
response header wait phase in `FixedHTTP2Connection.handle_async_request()`.
Unlike the socket-level `read` timeout (which resets on each `socket.receive()`
and never fires while other streams keep the socket busy), the per-stream
deadline is enforced at the Python event-loop level via `asyncio.wait_for()`
and fires regardless of activity on other streams.
