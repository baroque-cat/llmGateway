...
class KeyProbe(IResourceProbe):
    ...
    async def _check_resource(self, resource: dict[str, Any]) -> CheckResult:
        ...
        logger.debug(
            f'Checking key (ID: {key_id}) for provider "{provider_name}", model "{actual_model_name}" (requested: "{model_name}").'
        )
        ...
        try:
            ...
            if health_policy is None:
                logger.warning(
                    f'Health policy not found for provider "{provider_name}". Using default policy.'
                )
            ...
            if result.error_reason.is_retryable():
                ...
                for attempt in range(health_policy.verification_attempts):
                    ...
                    logger.debug(
                        f"Verification attempt {attempt + 1} for key ID {key_id}"
                    )
            ...
            logger.debug(
                f"Key ID {key_id} failed with non-retryable error {result.error_reason}. No verification."
            )
            return result
    ...
    def _calculate_next_check_time(
        ...
        if reason == ErrorReason.INVALID_KEY:
            ...
            return now + timedelta(
                hours=policy.on_rate_limit_hr
            )  # Using the new _hr field
